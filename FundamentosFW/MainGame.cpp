...
void MainGame::initLevel() {
	...
	_player = new Player();
	_currenLevel = 0;
	_player->init(PLAYER_SPEED, _levels[_currenLevel]->getPlayerPosition(), &_inputManager, &_camera, &_bullets);
	...
	_player->addGun(new Gun("Magnum", 10, 1, 5.0f, 30, 1.0f));
}
...
void MainGame::updateAgents(float totalDeltime) {
	...
	for (size_t i = 0; i < _bullets.size(); i++)
	{
		_bullets[i].update(_levels[_currenLevel]->getLevelData(), totalDeltime);
	}
	for (size_t i = 0; i < _humans.size(); i++)
	{
